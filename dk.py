...
def _calculate(coeffs, zeroes, model, max_iter, tolerance):
    ...
    mask = np.ones(
        num_polys, dtype=bool
    )  # True/1 means active polynomial (not converged)
    ...
    for iteration in range(max_iter):
        if iteration % 100 == 0:
            ...
            logging.info(
                f" calculated polynomials: {num_polys - np.sum(mask)}/{num_polys} ".center(
                    50, "-"
                )
            )
        ...
        new_f0 = np.array(
            [np.polyval(active_c[i], new_r[i]) for i in range(len(active_c))]
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Durand-Kerner algorithm for evaluation"
    )
    parser.add_argument(
        "-m",
        "--model",
    ...
    iterations = Parallel(n_jobs=args.cores)(
        delayed(
            lambda z: _calculate(
                z[0], z[1], model, max_iter, CONFIG["evaluation"]["dk_tolerance"]
            )
        )(z)
        for z in zip(coeffs_splitted, zeroes_splitted)
    )
    ...
    logging.info(
        f"polynomials_out_of_iterations: {len(iterations[iterations >= max_iter])}"
    )
    logging.info(
        f"iterations_completed_total: {np.sum(iterations[iterations < max_iter])}"
    )
